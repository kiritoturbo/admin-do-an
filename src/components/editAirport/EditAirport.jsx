...
// import { TextField, Select, Button, FormControl, InputLabel, FormHelperText} from '@mui/material';
import {
  TextField,
  Select,
  Button,
...
function EditAirport(props) {
  useEffect(() => {
    ...
    props.fetchAirport(props.idss);
  }, []);

  const onSubmit = (formValues) => {
    // props.editAirport(props?.airport._id, formValues);
    props.editAirport(props.idss, formValues);
  };

  if (!props.airport) return <div>Loading...</div>;
...
const validate = (formValues) => {
  const error = {};
  const requiredFields = ["name", "location", "nation"];
...
const mapStateToProps = (state, ownProps) => {
  ...
  const airport = state?.airports[ownProps?.idss];
  return {
    airport: airport,
    initialValues: {
      name: airport?.name,
...
export default connect(mapStateToProps, { editAirport, fetchAirport })(
  newAirportForm
);

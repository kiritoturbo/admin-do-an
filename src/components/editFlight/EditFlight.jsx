...
function EditFlight(props) {
  console.log("props ở edit flight" + props.match?.params.id);
  useEffect(() => {
    props.fetchAirports();
    props.fetchAirliners();
    // props.fetchFlight(props.match?.params.id);
    props.fetchFlight(props.idss);
  }, []);

  const onSubmit = (formValues) => {
    // props.editFlight(props.flight._id, {
    props.editFlight(props.idss, {
      ...formValues,
      price: {
  ...
  if (!props.flight) return <div>Loading...</div>;
  return (
            ...
            <div className="left">
              ...
              <Field name="airliner" component={renderSelect} label="Airliner">
                ...
                {props.airliners.map((airliner) => (
                  <option key={airliner._id} value={airliner._id}>
                    {airliner.manufacturer + airliner.model}
                  </option>
                ))}
              </Field>

...
import { fetchHotels, editHotel } from "../../action/index.js";
...
import { modules } from "../moduleToolbar.js";
  ...
  const singlePostById = async () => {
    ...
    try {
      const { data } = await axios.get(
        `http://localhost:2020/banner/post/${id}`,
        {
          headers: {
  ...
  const updatePost = async (values) => {
    try {
      const { data } = await axios.put(
        `http://localhost:2020/banner/update/post/${id}`,
        values,
        {
          headers: {
            authorization: "Bearer " + store.getState().auth.accessToken,

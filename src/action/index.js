...
//     });
//   } catch (error) {
//     console.log("lỗi ở fetchAirport" + error);
//   }
// };
export const fetchAirport = (airportId) => async (dispatch) => {
  try {
...
export const editHotel = (hotelId, formValues) => async (dispatch) => {
  try {
    ...
    <Navigate to={"/hotels"} />;
    dispatch(showAlert("Update Hotel success!", "success"));
    setTimeout(() => {
      dispatch(hideAlert());
    }, 5000);
  } catch (err) {
...
export const deleteHotel = (hotelId) => async (dispatch) => {
  try {
    ...
    dispatch(showAlert("Delete hotel success!", "success"));
    setTimeout(() => {
      dispatch(hideAlert());
    }, 5000);
  } catch (err) {
...
// export const editRoom = (hotelId, formValues) => async (dispatch) => {
//   try {
//     const res = await axiosJWT.put(`/api/hotels/${hotelId}`, formValues, {
//       headers: {
//         authorization: "Bearer " + store.getState().auth.accessToken,
...
export const deleteRoom = (roomId, hotelId) => async (dispatch) => {
  ...
  if (res.status === 200) {
    ...
    dispatch(showAlert("Delete room success!", "success"));
    setTimeout(() => {
      dispatch(hideAlert());
    }, 5000);
    history.push("/rooms");
